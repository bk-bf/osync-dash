#!/usr/bin/env python3
"""osd — terminal dashboard for osync sync jobs (core / one-shot renderer).

Reads the compose file (~/.config/osync/osd.toml), which defines many
...
net. This core is pure Python stdlib; the interactive TUI (osd_tui.py) adds
Textual.
...
def _resolve_osync() -> str:
    ...
    home = os.environ.get("OSD_HOME", os.path.expanduser("~/.local/share/osd"))
...
COMPOSE_FILE = CONFIG_HOME / "osd.toml"
_LEGACY_COMPOSE = CONFIG_HOME / "osync-dash.toml"  # pre-rename name


def _migrate_compose() -> None:
    """Carry a pre-rename compose file over to the current name, once."""
    if _LEGACY_COMPOSE.exists() and not COMPOSE_FILE.exists():
        try:
            _LEGACY_COMPOSE.rename(COMPOSE_FILE)
        except OSError:
            pass
...
def pick_config(explicit: str | None) -> Path:
    ...
    if not confs:
        sys.exit(f"osd: no configs in {CONFIG_HOME} (use -c PATH)")
    if len(confs) == 1:
        return confs[0]
...
CONFIG_TEMPLATE = """\
###### osync configuration — generated by osd
...
## osd: endpoint bookkeeping (ignored by osync). Mode ts|ssh|both;
## in 'both', osd prefers Tailscale and falls back to SSH.
...
def dump_compose(defaults: dict, conns: list[dict], settings: dict | None = None) -> None:
    ...
    lines = ["# osd compose — one file, many sync connections.",
...
LAUNCHER = Path(__file__).resolve().parent / "osd"


def self_update() -> int:
    """`osd -U` — pull the source tree, then re-run its installer.

    Works for both install shapes: the curl installer leaves a git clone at
    ~/.local/share/osd/src, and a manual checkout is one already.
    """
    src = Path(__file__).resolve().parent
    installer = src / "install.sh"
    if not (src / ".git").exists() or not installer.exists():
        sys.stderr.write(
            f"osd: {src} is not a git checkout — reinstall with\n"
            f"     curl -fsSL https://raw.githubusercontent.com/bk-bf/"
    ...
    if rc != 0:
        ...
        return rc
    # The installer is idempotent and re-links the launchers, so it is also how
    # a rename or a new command in the tree reaches ~/.local/bin.
    return subprocess.call(["bash", str(installer)])
...
_SVC_CHANGE = """\
[Unit]
Description=osd continuous sync — {name}
...
_SVC_ONESHOT = """\
[Unit]
Description=osd scheduled sync — {name}
After=network-online.target
Wants=network-online.target
...
_TIMER = """\
[Unit]
Description=osd periodic sync timer — {name}
...
_TIMER_CAL = """\
[Unit]
Description=osd scheduled sync timer — {name}
...
def main():
    ...
    while i < len(args):
        ...
        if a in ("-h", "--help"):
            ...
            sys.exit(f"osd: unknown option {a} (see --help)")
        i += 1
    ...
    if not conns:
        sys.exit(f"osd: no connections. Add one in {COMPOSE_FILE} "
    ...
    if o["log"]:
        ...
        if logf and os.path.exists(logf):
            ...
            sys.exit("osd: no log file found")
        return
    ...
    # osd_core only does the one-shot render; the interactive TUI lives in
    # osd_tui.py (Textual) and is launched by the osd wrapper.
